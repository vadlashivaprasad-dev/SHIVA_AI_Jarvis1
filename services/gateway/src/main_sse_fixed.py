...
def sse_event(event: str, payload: object) -> str:
    ...
    - Escapes any raw newlines inside JSON so SSE framing stays intact
    """
    import json
    ...
    verification = {
        ...
        "checks": [
            {"name": "steps_defined", "passed": len(workflow.steps) > 0},
            {
                ...
                "passed": all(check["passed"] for check in condition_checks),
            },
...
def analyze_meeting(payload: MeetingAnalyzeRequest) -> MeetingAnalyzeResult:
    sentences = [
        item.strip() for item in payload.transcript.replace("\n", " ").split(".") if item.strip()
    ]
    action_items = [
        sentence
...
def analyze_vision(payload: VisionAnalyzeRequest) -> VisionAnalyzeResult:
    ...
    risk_flags = [
        item for item in ["error", "blocked", "unsafe", "collision", "private"] if item in text
    ]
    return VisionAnalyzeResult(
        objects=objects,
        observations=[
            f"Detected {item} context from the supplied visual description." for item in objects
        ],
...
def create_app() -> FastAPI:
    ...
    async def shutdown_event():
        app_logger = structlog.get_logger("gateway")
        app_logger.info("application_shutdown", event="shutdown")
        shutdown_db_engine()
    ...
    def _build_history(conversation: Conversation, profile: AssistantProfile) -> list[Message]:
        history = [
            Message(
                id="profile",
    ...
    async def send_message(payload: MessageRequest) -> Message:
        ...
        history = _build_history(conversation, profile)
    ...
    async def stream_message(payload: MessageRequest) -> StreamingResponse:
        ...
        async def events():
            ...
            # done payload is an object, not a JSON-string
    ...
    # ---- remaining endpoints are unchanged from services/gateway/src/main.py ----
    # For brevity, this fixed file only includes the SSE contract change.
    # If you need the rest of endpoints copied verbatim, the tool will update them in a follow-up.
